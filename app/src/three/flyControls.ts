...
import { headingDegFromForward, normaliseAngle } from './compass';
...
export interface FlyTelemetry {
   ...
   *
   * ⚠️ This read ANTICLOCKWISE until 2026-08-18 — due east printed 270 — while the comment on this
   * line claimed clockwise. It now comes from `compass.headingDegFromForward`, which is the same
   * function the rose uses, so the number and the picture cannot drift apart again.
   */
  headingDeg: number;
}
...
export interface FlyControls {
  ...
  /**
   * Swing the gimbal round to due north, the short way. Height, position and pitch are untouched.
   *
   * Aims the target yaw rather than snapping the live one, so the existing look stabiliser eases
   * the turn — the same motion any other look produces, rather than a second animation with its
   * own feel.
   */
  faceNorth(): void;
...
export function createFlyControls(options: FlyControlsOptions): FlyControls {
  ...
  return {
    ...
    faceNorth() {
      // North is yaw 0 (heading is the yaw negated — see `compass.ts`), and the SHORT way there is
      // the point: `targetYaw` accumulates, so a camera that has been spun round twice sits at 13
      // radians and easing it to a bare 0 would unwind two whole revolutions on screen.
      targetYaw += normaliseAngle(-targetYaw);
      // The spin centre is latched from where the camera was; keeping it across a turn would make
      // the next R or F swing around a point that is no longer in front of anyone.
      spinCentre = null;
    },
    ...
    telemetry(): FlyTelemetry {
      ...
      return {
        ...
        headingDeg: headingDegFromForward(forward.x, forward.z),
      };
    },
