...
import {
  ...
  type FlyControls,
  type OrbitLike,
} from '../src/three/flyControls.js';

/*
 * Ported from the twin apps that share this module. Exactly two lines differ
 * from their copy: the import path above, because this repo keeps tests in a
 * `test` folder rather than beside the source, and the jsdom docblock, because
 * the rest of this repo's tests run in node and should not pay for a DOM.
 *
 * Everything else is deliberately untouched, so a future fix in any of the
 * other copies can be diffed straight against this one.
 */
